    ...
    <div class="container"> 
        <div class="row">
          <div class="text-center">
            <h2 class="site-title" style="font-size:40px; font-family:Verdana, Geneva, Tahoma, sans-serif; color:#DC143C">Countries</h2>
          </div>
        </div>
    </div>
<table>
  ...
  <?php
      ...
      if($result->num_rows>0){
        while($row=$result->fetch_assoc()){
          echo "<tr><td>".$row["Countryabb"]."</td><td>".$row["Country"]."</td><td>".$row["Goldmedals"]."</td><td>".$row["Silvermedals"]."</td><td>".$row["Bronzemedals"]."</td><td>".$row["Totalmedals"]."</td></tr>";
        }
      }
        else{
          echo "no results";
